...
async def get_h2h(request: TeamsRequest):
    return H2H_stats(request.team_1, request.team_2, request.league)

@router.post("/h2h/latest")
async def get_latest_h2h(request: TeamsRequest):
    return latest_H2H(request.team_1, request.team_2, request.league)

@router.post("/recent")
async def get_recent_matches(request: TeamsRequest):
    return recent_matches(request.team_1, request.team_2, request.league)